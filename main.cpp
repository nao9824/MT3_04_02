...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		if (startButton) {
			pendulum.angulerAcceleration = -(9.8f / pendulum.lenght) * std::sin(pendulum.angle);
			pendulum.angularVelocity += pendulum.angulerAcceleration * deltaTime;
		...
		Vector3 pendulumNdcVertex = Transform(pendulum.anchor, worldViewProjectionMatrix);
